...
const {validateData, existEntity, entities} = require('../utils/validate.utils');
...
exports.newEntity = async(req, res) => {
    try{
        ...
        if(!msg) {
            ...
            const entidad = nombreExist.find(entidad => entidad.nombreEntidad === data.nombreEntidad);
            if(!entidad) {
...
exports.putEntity = async(req, res) => {
    try{    
        ...
        const entidad = entityExist.find(entidad => entidad.codigoNIT == req.params.idEntity);
        const already = nombreExist.find(entidad => entidad.nombreEntidad === req.body.nombreEntidad);
...
exports.deleteEntity = async(req, res) => {
    try{
        ...
        console.log(err);
        return res.status(500).send({message: 'Error en el servidor deleteEntity'});
    }
}
...
exports.entityById = async(req, res) => {
    try{
        ...
        const entidad = entidadExist.find(entidad => entidad.codigoNIT == req.params.idEntity);
        if(!entidad) return res.status(404).send({message: 'Entity not found'});
